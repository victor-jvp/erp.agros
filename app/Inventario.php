    ...
    public function scopeStock($query)
    {
        $query = 'SELECT
                    ...
                    (CASE 
                        ...
                        ELSE ""
                    END) AS material,
                    (SELECT SUM(inventario.cantidad) FROM inventario WHERE inventario.tipo_mov = "E" AND inventario.categoria_id = inv.categoria_id AND inventario.categoria = inv.categoria) AS entradas,
                    (SELECT SUM(inventario.cantidad) FROM inventario WHERE inventario.tipo_mov = "S" AND inventario.categoria_id = inv.categoria_id AND inventario.categoria = inv.categoria) AS salidas
                FROM
                    inventario as inv
                GROUP BY
                    inv.categoria,
